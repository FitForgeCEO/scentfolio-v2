  ...
  return (
    <div className="space-y-3 animate-pulse">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-surface-container rounded-sm p-4 flex items-center gap-3">
          <div className="w-10 h-10 rounded-sm bg-surface-container-highest flex-shrink-0" />
...
export function FragranceListSkeleton({ count = 5 }: { count?: number }) {
  return (
    <div className="space-y-2 animate-pulse">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-surface-container rounded-sm p-4 flex items-center gap-3">
          <div className="w-12 h-12 rounded-sm bg-surface-container-highest flex-shrink-0" />
          <div className="flex-1 space-y-2">
...
export function HeroListSkeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      {/* Hero card */}
      <div className="bg-surface-container rounded-sm p-6 flex flex-col items-center gap-4">
        <div className="w-16 h-4 bg-surface-container-highest rounded" />
        <div className="w-20 h-20 rounded-sm bg-surface-container-highest" />
  ...
  return (
    <div className="grid grid-cols-2 gap-3 animate-pulse">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-surface-container rounded-sm p-5 space-y-2">
