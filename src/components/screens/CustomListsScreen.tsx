...
export function CustomListsScreen() {
    ...
    return (
      <main className="pt-24 pb-32 px-6 max-w-[430px] mx-auto min-h-screen flex flex-col items-center justify-center">
        <div className="w-16 h-16 rounded-full bg-surface-container flex items-center justify-center mb-5">
          <Icon name="bookmarks" className="text-3xl text-primary/40" />
        </div>
  ...
  return (
    <main className="pt-24 pb-32 px-6 max-w-[430px] mx-auto min-h-screen space-y-6">
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 rounded-xl bg-surface-container animate-pulse" />)}</div>
      ) : lists.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16">
          <div className="w-20 h-20 rounded-full bg-surface-container flex items-center justify-center mb-6">
            <Icon name="bookmarks" className="text-primary/40 text-4xl" />
          </div>
        ...
        <div className="space-y-3">
          {lists.map((list) => (
            <div key={list.id} className="bg-surface-container rounded-xl overflow-hidden">
              <button
                onClick={() => navigate(`/lists/${list.id}`)}
                className="w-full flex items-center gap-4 px-4 py-4 text-left active:bg-surface-container-highest transition-colors"
              >
                <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${list.color}20` }}>
                  <Icon name={list.icon} style={{ color: list.color }} size={20} />
                </div>
                ...
                <Icon name="chevron_right" className="text-secondary/40" size={18} />
              </button>
              <div className="flex border-t border-outline-variant/10">
                <button
                  onClick={() => handleDelete(list.id)}
                  className="flex-1 py-2 text-[10px] text-error/60 font-bold tracking-widest uppercase active:bg-error/5 transition-colors"
                >
                  DELETE
                </button>
              </div>
        ...
        <div className="px-8 pb-10 space-y-6">
          ...
          <div className="flex items-center gap-3 py-3">
            <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: `${color}20` }}>
              <Icon name={icon} style={{ color }} size={24} />
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {ICON_OPTIONS.map((ic) => (
                <button
                  key={ic}
                  onClick={() => setIcon(ic)}
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${icon === ic ? 'ring-2 ring-primary bg-primary/10' : 'bg-surface-container'}`}
                >
                  <Icon name={ic} style={{ color: icon === ic ? color : undefined }} className={icon === ic ? '' : 'text-secondary/60'} size={18} />
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="flex gap-3">
              {COLOR_OPTIONS.map((c) => (
                <button
                  key={c}
                  onClick={() => setColor(c)}
                  className={`w-8 h-8 rounded-full transition-all ${color === c ? 'ring-2 ring-offset-2 ring-offset-surface-container-low ring-primary' : ''}`}
  ...
  if (loading) {
    return (
      <main className="pt-24 pb-32 px-6 max-w-[430px] mx-auto min-h-screen">
        <div className="space-y-4 pt-4">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 rounded-xl bg-surface-container animate-pulse" />)}</div>
      </main>
    )
  }
    ...
    <main className="pt-24 pb-32 px-6 max-w-[430px] mx-auto min-h-screen space-y-6">
      {list && (
        <header className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ backgroundColor: `${list.color}20` }}>
            <Icon name={list.icon} style={{ color: list.color }} size={24} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {items.map((item) => (
            <div key={item.id} className="relative space-y-2 group">
              <div
                className="aspect-[3/4] rounded-xl overflow-hidden bg-surface-container-low cursor-pointer"
              ...
              <button
                ...
                className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center active:scale-90 transition-transform opacity-0 group-hover:opacity-100"
              >
                <Icon name="close" className="text-white" size={14} />
              </button>
              <div>
    ...
    <div ref={trapRef} className="fixed inset-0 z-[var(--z-sheet)] flex flex-col justify-end" role="dialog" aria-modal="true">
      ...
      <section className="relative w-full max-h-[70vh] bg-surface-container-low rounded-t-[2.5rem] sheet-shadow flex flex-col overflow-hidden animate-slide-up">
        ...
        <div className="px-8 pb-4">
          <div className="flex items-center bg-surface-container rounded-2xl px-4 py-3 focus-within:ring-1 ring-primary/30">
              ...
              {results.map((f) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 py-3 text-left disabled:opacity-40 active:bg-surface-container-highest transition-colors"
                  >
