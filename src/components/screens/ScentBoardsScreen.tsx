  ...
  return (
    <main className="pt-24 pb-32 px-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setCreateOpen(true)} className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center active:scale-90 transition-transform">
      ...
      {boards.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center pt-16 space-y-4">
          <div className="w-16 h-16 rounded-full bg-surface-container flex items-center justify-center">
            <Icon name="dashboard" className="text-2xl text-secondary/40" />
          </div>
          ...
          <button onClick={() => setCreateOpen(true)} className="gold-gradient text-on-primary-container px-6 py-3 rounded-xl font-label text-[10px] font-bold uppercase tracking-widest">
        ...
        <div className="space-y-3">
          {boards.map((board, i) => (
            <button key={board.id} onClick={() => navigate(`/boards/${board.id}`)} className="w-full rounded-xl overflow-hidden active:scale-[0.98] transition-transform text-left">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {board.is_public && (
                    <>
                      ...
                      <Icon name="public" className="text-primary/50 text-xs" />
                    </>
                  )}
                </div>
              </div>
      ...
      <section className="relative w-full bg-surface-container-low rounded-t-[2.5rem] sheet-shadow flex flex-col overflow-hidden animate-slide-up">
        ...
        <div className="px-8 space-y-6 pb-10">
          <div className="space-y-2">
            ...
            <input className="w-full bg-surface-container border-none text-on-surface placeholder:text-on-surface-variant/40 rounded-2xl p-4 text-sm focus:ring-1 focus:ring-primary/30 focus:outline-none" placeholder="e.g. Date Night, Summer Rotation..." value={title} onChange={(e) => setTitle(e.target.value)} autoFocus />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] uppercase tracking-[0.2em] text-primary font-bold">DESCRIPTION (OPTIONAL)</label>
            <textarea className="w-full bg-surface-container border-none text-on-surface placeholder:text-on-surface-variant/40 rounded-2xl p-4 text-sm focus:ring-1 focus:ring-primary/30 focus:outline-none resize-none" placeholder="What's this board about?" rows={3} value={description} onChange={(e) => setDescription(e.target.value)} />
          </div>
          <button
    ...
    <main className="pt-24 pb-32 px-6 space-y-6">
      <div>
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={() => setAddSheetOpen(true)} className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full active:scale-95 transition-transform">
            <Icon name="add" className="text-primary text-sm" />
            <span className="text-[10px] font-bold tracking-widest text-primary uppercase">ADD</span>
          </button>
          <button onClick={() => setConfirmDelete(true)} className="flex items-center gap-2 bg-error/10 px-4 py-2 rounded-full active:scale-95 transition-transform">
            <Icon name="delete" className="text-error/70 text-sm" />
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center pt-12 space-y-4">
          <div className="w-16 h-16 rounded-full bg-surface-container flex items-center justify-center">
            <Icon name="water_drop" className="text-2xl text-secondary/40" />
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {items.map((item) => (
            <div key={item.id} className="space-y-2 group cursor-pointer" role="link" tabIndex={0} onClick={() => navigate(`/fragrance/${item.fragrance.id}`)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/fragrance/${item.fragrance.id}`) } }}>
              <div className="aspect-[3/4] rounded-xl overflow-hidden bg-surface-container-low shadow-sm relative">
                {item.fragrance.image_url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-secondary/50"><Icon name="water_drop" size={48} /></div>
                )}
                <button onClick={(e) => { e.stopPropagation(); handleRemoveItem(item.id) }} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 active:opacity-100 transition-opacity active:scale-90" aria-label="Remove from board">
              ...
              {results.map((f) => (
                <button key={f.id} onClick={async () => { setSaving(true); await addToBoard(boardId, f.id); setSaving(false); onAdded() }} disabled={saving} className="w-full flex items-center gap-3 p-3 hover:bg-surface-container-highest active:bg-surface-container-highest transition-colors text-left disabled:opacity-50">
                  <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-surface-container-highest">
                    {f.image_url ? <img src={f.image_url} alt={f.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Icon name="water_drop" className="text-secondary/30" size={16} /></div>}
                  </div>
...
function DeleteConfirmDialog({ boardTitle, onCancel, onConfirm }: { boardTitle: string; onCancel: () => void; onConfirm: () => void }) {
  ...
  return (
    <div ref={trapRef} className="fixed inset-0 z-[var(--z-modal)] flex items-center justify-center px-8" role="alertdialog" aria-modal="true" aria-label="Delete board">
      ...
      <div className="relative bg-surface-container-low rounded-2xl p-6 max-w-[320px] w-full space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 py-3 rounded-xl bg-surface-container text-sm font-medium text-on-surface active:scale-95 transition-transform">Cancel</button>
          <button onClick={async () => { setDeleting(true); await onConfirm() }} disabled={deleting} className="flex-1 py-3 rounded-xl bg-error/20 text-sm font-medium text-error active:scale-95 transition-transform disabled:opacity-50">
